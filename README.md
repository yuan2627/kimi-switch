...
基于本机 58 次真实切换与 3.6 万对相邻请求的统计（K3、相邻调用间隔 <15 分钟）：约 3/4 的换号后调用缓存照常命中，但缓存命中率暴跌（<50%）的概率从平时的约 0.4% 上升到约 17%–19%。建议在大上下文会话的关键节点避免中途换号；若必须切换，换号后第一轮请求可能按全量计费。

（注：本地日志无法逐请求标记账号，此结论为时间对齐的统计相关性而非严格因果；上下文重建等因素也会独立导致缓存失效。）
